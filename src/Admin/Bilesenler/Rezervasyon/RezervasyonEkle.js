...
const RezervasyonEkle = ({ acik, kapat, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.masaId || !formData.musteriAdi || !formData.tarih || !formData.saat) {
    ...
    try {
      ...
      setFormData({ masaId: '', musteriAdi: '', tarih: '', saat: '' });
      kapat();
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 bg-white hover:bg-gray-200 text-black font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {loading ? <><div className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin"></div> Ekleniyor...</> : 'Ekle'}
            </button>
          </div>
        </form>
      </div>
